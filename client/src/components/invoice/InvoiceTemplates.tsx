...
// Cloudflare-style Template - Clean, minimal design inspired by Cloudflare invoices
export function CloudflareTemplate({ logo, invoiceData, paymentDetails }: TemplateProps) {
  const subtotal = invoiceData.items.reduce((sum, item) => sum + item.quantity * item.price, 0);
            ...
            <div className="space-y-2">
              {paymentDetails.cryptoDetails.currency && (
                <div className="grid grid-cols-[140px_1fr]">
                  ...
                  <span className="text-gray-900">{paymentDetails.cryptoDetails.currency}</span>
                </div>
              )}
              {paymentDetails.cryptoDetails.network && (
                <div className="grid grid-cols-[140px_1fr]">
                  <span className="text-gray-700">Network:</span>
                  <span className="text-gray-900">{paymentDetails.cryptoDetails.network}</span>
                </div>
              )}
              {paymentDetails.cryptoDetails.walletAddress && (
                <div className="grid grid-cols-[140px_1fr]">
                  <span className="text-gray-700">Wallet Address:</span>
                  <span className="text-gray-900 font-mono text-xs break-all">{paymentDetails.cryptoDetails.walletAddress}</span>
                </div>
              )}
            </div>
          )}
